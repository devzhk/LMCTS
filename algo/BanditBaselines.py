...
from bandits.BanditTools import *
...
class LinUCB:
    ...
    def chooseArmToPlay(self):
        with torch.no_grad():
            norms = torch.Tensor([torch.sqrt(torch.matmul(torch.matmul(
                feature, self.DesignInv), feature.T)) for feature in self.features])
            # norms=np.array([np.sqrt(feature@self.DesignInv@feature.T) for feature in self.features])
            UCBs = torch.matmul(
                self.features, self.thetaLS.view(-1))+self.beta(self.t)*norms
            return torch.argmax(UCBs).item()

    def receiveReward(self, arm, reward):
        with torch.no_grad():
            x = self.features[arm, :].view(
                (self.dimension, 1))  # column vector
            ...
            self.DesignInv = self.DesignInv - \
                torch.matmul(omega, omega.T) / \
                (1+torch.matmul(x.T, omega).item())
...
class LinUCBClassifier:
    ...
    def clear(self):
        with torch.no_grad():
            ...
            # regularized least-squares estimate
            self.thetaLS = torch.zeros((self.d, 1))
